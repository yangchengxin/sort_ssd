...
{
    /* 初始化，在整个算法流程中只有一次，然后就把optimization_initialized_设置成true了 */
    OptimizationInit();

    /* 执行KM算法（带权重的二分图最优匹配算法） */
    DoMunkres();
...
void HungarianOptimizer<T>::DoMunkres()
{
    ...
    int num_iter = 0;
    /* 将HungarianOptimizer::ReduceRows函数绑定在fn_state对象上，可以通过执行fn_state()来执行这个函数 */
    fn_state_ = std::bind(&HungarianOptimizer::ReduceRows, this);

    /* while循环会执行完KM算法的所有步骤，每一个步骤都是通过fn_state_()函数来调用 */
 ...
 * from every element in its row. Then, go to Step 2. */
 ...
 /*
 *   cost matrix的每一行的元素依次减去这一行的最小值
 *   并将下一步操作的函数绑定在fn_state()上
 */
template <typename T>
