...
if __name__ == "__main__":
    ...
    #-------------------------------------------------------------------------#
    #   track               指定是否启动目标跟踪任务，false为不启动，true为启动
    #-------------------------------------------------------------------------#
    track = True
    ...
    if mode == "predict":
        ...
        '''
        ref 返回的是一个bool值，如果视频帧被正确读取，那么ret就是True，当视频帧读到最后一帧或者无法读取时，ref会变成false。
        '''
        ...
        '''
        if track == True:
            mot_tracker = sort.Sort(max_age=1, min_hits=3, iou_threshold=0.3)
        else:
            mot_tracker = None
        while(True):
            t1 = time.time()
            ...
            # 转变成Image，将opencv的mat变量变成PIL的Image
            '''
            输入视频流
            '''
            ...
            '''
            # 进行检测
            frame = np.array(ssd.detect_image(frame, mot_tracker))
