...
class SSD(object):
    ...
    #   检测图片
    #---------------------------------------------------#
    def detect_image(self, image, mot_tracker = None, crop = False, count = False):
        #---------------------------------------------------#
        #   计算输入图片的高和宽
        #---------------------------------------------------#
        ...
        with torch.no_grad():
            ...
            #--------------------------------------#
            #   top_label: 当前帧检测出来的目标类别索引
            #   top_conf : 当前帧检测出来的目标置信度
            #   top_boxes: 当前帧检测出来的目标框xyxy
            #--------------------------------------#
            ...
            '''
            torch.tensor的切片操作：跳过中间的元素， 组成的向量为[x,y,x,y,conf]
            '''
        ...
        #   sort跟踪
        #---------------------------------------------------------#
        # mot_tracker = sort.Sort(max_age=10, min_hits=3, iou_threshold=0.3)
        '''
        跟踪结果，[x,y,x,y,id]
        '''
        if mot_tracker != None:
            trackers    = mot_tracker.update(top_sort)
        #---------------------------------------------------------#
        #   设置字体与边框厚度
        #---------------------------------------------------------#
